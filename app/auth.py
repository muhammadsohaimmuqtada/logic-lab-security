    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not org:
                ...
                org_id = org["id"]
            cur = db.execute(
                "INSERT INTO users(username,email,password_hash,credits,referral_code,active_org_id) VALUES(?,?,?,?,?,?)",
                (username, email, generate_password_hash(password), 10000, f"{username.upper()}10", org_id),
            )
            uid = cur.lastrowid
    ...
    if request.method == "POST":
        ...
        user = get_db().execute(
            """SELECT u.* FROM users u
               ...
               WHERE u.username=? AND o.name=? COLLATE NOCASE""",
            (username, org_name),
        ).fetchone()
        if not user:
        ...
        flash(f"Password reset completed. Recovery audit code: {flag('LL16')}", "success")
        return redirect(url_for("auth.login"))
